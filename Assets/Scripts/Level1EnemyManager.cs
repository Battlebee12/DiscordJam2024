...
public class Level1EnemyManager : MonoBehaviour
{
    ...
    [SerializeField] private LvlHeart nerve;
    ...
    {   
        ...
        if(nerve.Health/nerve.MaxHealth > 0.5f ){
            spawnFrequency = 1;
        }
        if(nerve.Health/nerve.MaxHealth > 0.25){
            spawnFrequency =2;
        }
        if(nerve.Health/nerve.MaxHealth > 0.05){
            spawnFrequency =4;
        }
    ...
    private void SpawnTheEnimies(){
        if(currentEnimies < maxEnimies){
            enemySpawner.SpawnEnemyLvl1(GetRandomPointOnCircle(playerTransform.position,spawnRadius));
        }
